...
def update_order():
    ...
    query = "UPDATE orders SET order_date=%s, sender_address=%s, receiver_address=%s, description=%s, tracking_number=%s WHERE order_id=%s"
    values = (order_date, sender_address, receiver_address, description, tracking_number, order_id)
